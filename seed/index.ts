...
export async function seedDemo(): Promise<SeedSummary> {
  ...
  return {
    ...
    script: [
      ...
      `  from the term mean and from every released report card.`,
      ``,
      `Now sign in as the PARENT and open the portal. Two children, one login —`,
      `  their balance, the account number to pay to, and last term's released`,
      `  report cards. The unpublished exam above is not there. Publish it,`,
      `  recompute, and refresh: now it is. That is the whole gate, from both`,
      `  sides, in one minute.`,
      ``,
